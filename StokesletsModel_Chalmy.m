...
%theta=[0;  1*pi/2];
%dr0_l=[cos(theta(1))*cos(theta(2));   sin(theta(1))*cos(theta(2));   sin(theta(2))];     % body direction
dr0_l=[0 ; 0;  1];                      
...
%     abwn(:,i)=[w0x_k; w0y_k; w0z_k];
    ...
    v_orbit1         =  quater(v_orbit1,dr0_l,w0x_k*dt);
    v_orbit2         =  quater(v_orbit2,dr0_l,w0x_k*dt);
    vn1_0            =  quater(vn1_0,dr0_l,w0x_k*dt);
    vn2_0            =  quater(vn2_0,dr0_l,w0x_k*dt);
    ...
    v_orbit1         =  quater(v_orbit1,dyaw_l,w0y_k*dt);
    v_orbit2         =  quater(v_orbit2,dyaw_l,w0y_k*dt);
    vn1_0            =  quater(vn1_0,dyaw_l,w0y_k*dt);
    vn2_0            =  quater(vn2_0,dyaw_l,w0y_k*dt);
    ...
    v_orbit1         =  quater(v_orbit1,dp_l,w0z_k*dt);
    v_orbit2         =  quater(v_orbit2,dp_l,w0z_k*dt);
    vn1_0            =  quater(vn1_0,dp_l,w0z_k*dt);
    vn2_0            =  quater(vn2_0,dp_l,w0z_k*dt);
...
plot3(xn(1,1:k),xn(2,1:k),xn(3,1:k),'color',[0 0 1],'linewidth',2);
...
% draw body
Chlamyplot(xc,yc,zc,bodylength,thickness,bodyrot,bodyangle,eyerot,eyeangle,0.6,1/0.6)     
